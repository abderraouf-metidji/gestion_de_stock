...
def ajouter_produit():
    ...
    if nom != "" and description != "" and prix != "" and quantite != "" and categorie != "":
        ...
        cursor.close()
        messagebox.showinfo("Produit ajouté", "Le produit a été ajouté avec succès !")
...
def modifier_quantite():
    ...
    if id_produit != "" and quantite != "":
        ...
        if produit:
            ...
            status_label.config(text="Quantité mise à jour avec succès !", fg="green")
        else:
...
categorie_menu = tk.OptionMenu(root, categorie_var, *[cat[0] for cat in categories])
categorie_menu.grid(row=4, column=1, padx=5, pady=5, sticky=tk.W)

#création des boutons pour ajouter et afficher les produits

afficher_button = tk.Button(root, text="Afficher les produits", command=afficher_produits)
...
modifier_button = tk.Button(root, text="Modifier la quantité", command=modifier_quantite)
modifier_button.grid(row=8, column=0, padx=5, pady=5)

supprimer_button = tk.Button(root, text="Supprimer un produit", command=supprimer_produit)
